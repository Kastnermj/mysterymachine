...
DETAIL_PATTERNS = [
    ...
    (
        ...
        85,
        [
            r"\bfiled (?:for )?(?:voluntary )?bankruptcy\b",
            r"\bcommenced (?:a )?(?:voluntary )?(?:case|proceeding) under chapter 11\b",
            r"\bchapter 11 (?:case|petition|proceeding|bankruptcy)\b",
            r"\bpetition (?:for|under) (?:chapter 11|bankruptcy)\b",
            r"\bentered (?:into )?receivership\b",
            r"\breceiver (?:was|has been) appointed\b",
        ],
    ),
    (
        ...
        75,
        [
            r"\bevent of default (?:has|had) occurred\b",
            r"\bevent of default is continuing\b",
            r"\breceived (?:a )?notice of default\b",
            r"\bin default under\b",
            r"\bdefaulted (?:on|under)\b",
            r"\bdeclared .* immediately due and payable\b",
            r"\baccelerated (?:the )?(?:maturity|obligations|debt|loans)\b",
            r"\bacceleration of (?:the )?(?:maturity|obligations|debt|loans)\b",
        ],
    ),
    ("delisting_or_listing_failure", 38, 70, [r"\bdelisting\b", r"\bnasdaq.*non-compliance\b", r"\bminimum bid\b"]),
    (
        "going_concern_or_liquidity",
        34,
        65,
        [
            r"\bgoing concern\b",
            ...
            r"\bliquidity (?:crisis|shortfall|constraint|constraints|concern|concerns|risk|risks)\b",
            r"\bmay not have sufficient (?:cash|liquidity|capital)\b",
            r"\bwill require additional (?:capital|financing|liquidity) to continue\b",
            r"\bnot expected to have sufficient (?:cash|liquidity|capital)\b",
        ],
    ),
